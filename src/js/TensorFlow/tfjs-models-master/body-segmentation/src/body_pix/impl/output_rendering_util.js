 ...
 */
function toMask(personOrPartSegmentation, foreground = {
    r: 0,
    g: 0,
    b: 0,
    a: 0
    ...
    function drawStroke(bytes, row, column, width, radius, color = { r: 0, g: 255, b: 255, a: 255 }) {
    ...
    function isSegmentationBoundary(segmentationData, row, column, width, foregroundIds = [1], radius = 1) {
        ...
        for (let j = 0; j < width; j += 1) {
            ...
            for (let k = 0; k < multiPersonOrPartSegmentation.length; k++) {
                if (foregroundIds.some(id => id === multiPersonOrPartSegmentation[k].data[n])) {
                    ...
                    const isBoundary = isSegmentationBoundary(multiPersonOrPartSegmentation[k].data, i, j, width, foregroundIds);
 ...
 */
function toColoredPartMask(partSegmentation, partColors = RAINBOW_PART_COLORS) {
...
function drawMask(canvas, image, maskImage, maskOpacity = 0.7, maskBlurAmount = 0, flipHorizontal = false) {
    const [height, width] = (0, util_1.getInputSize)(image);
    canvas.width = width;
    canvas.height = height;
    const ctx = canvas.getContext('2d');
    ctx.save();
...
function createPersonMask(multiPersonSegmentation, edgeBlurAmount) {
    const backgroundMaskImage = toMask(multiPersonSegmentation, { r: 0, g: 0, b: 0, a: 255 }, { r: 0, g: 0, b: 0, a: 0 });
 ...
 * to false.
 */
function drawBokehEffect(canvas, image, multiPersonSegmentation, backgroundBlurAmount = 3, edgeBlurAmount = 3, flipHorizontal = false) {
...
function createBodyPartMask(multiPersonPartSegmentation, bodyPartIdsToMask, edgeBlurAmount) {
    const backgroundMaskImage = toMask(multiPersonPartSegmentation, { r: 0, g: 0, b: 0, a: 0 }, { r: 0, g: 0, b: 0, a: 255 }, true, bodyPartIdsToMask);
    const backgroundMask = renderImageDataToOffScreenCanvas(backgroundMaskImage, CANVAS_NAMES.mask);
    if (edgeBlurAmount === 0) {
        return backgroundMask;
    }
 ...
 * to false.
 */
function blurBodyPart(canvas, image, partSegmentation, bodyPartIdsToBlur = [0, 1], backgroundBlurAmount = 3, edgeBlurAmount = 3, flipHorizontal = false) {
    const blurredImage = drawAndBlurImageOnOffScreenCanvas(image, backgroundBlurAmount, CANVAS_NAMES.blurred);
    canvas.width = blurredImage.width;
    canvas.height = blurredImage.height;
    const ctx = canvas.getContext('2d');
    ...
    const bodyPartMask = createBodyPartMask(partSegmentation, bodyPartIdsToBlur, edgeBlurAmount);
    ctx.save();
    if (flipHorizontal) {
        flipCanvasHorizontal(canvas);
    }
