...
const decode_pose_1 = require("./decode_pose");
const util_1 = require("./util");
function withinNmsRadiusOfCorrespondingPoint(poses, squaredNmsRadius, { x, y }, keypointId) {
    return poses.some(({ keypoints }) => {
        const correspondingKeypoint = keypoints[keypointId].position;
        return (0, util_1.squaredDistance)(y, x, correspondingKeypoint.y, correspondingKeypoint.x) <=
            squaredNmsRadius;
    });
}
...
function decodeMultiplePoses(scoresBuffer, offsetsBuffer, displacementsFwdBuffer, displacementsBwdBuffer, outputStride, maxPoseDetections, scoreThreshold = 0.5, nmsRadius = 20) {
    const poses = [];
    const queue = (0, build_part_with_score_queue_1.buildPartWithScoreQueue)(scoreThreshold, kLocalMaximumRadius, scoresBuffer);
    ...
    while (poses.length < maxPoseDetections && !queue.empty()) {
        ...
        const rootImageCoords = (0, util_1.getImageCoords)(root.part, outputStride, offsetsBuffer);
        if (withinNmsRadiusOfCorrespondingPoint(poses, squaredNmsRadius, rootImageCoords, root.part.id)) {
            continue;
        }
        // Start a new detection instance at the position of the root.
        const keypoints = (0, decode_pose_1.decodePose)(root, scoresBuffer, offsetsBuffer, outputStride, displacementsFwdBuffer, displacementsBwdBuffer);
