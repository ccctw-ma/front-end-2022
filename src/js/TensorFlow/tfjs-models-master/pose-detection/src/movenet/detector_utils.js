...
function validateModelConfig(modelConfig) {
    const config = modelConfig == null ? constants_1.MOVENET_CONFIG : Object.assign({}, modelConfig);
...
function validateEstimationConfig(estimationConfig) {
    const config = estimationConfig == null ? constants_1.MOVENET_ESTIMATION_CONFIG : Object.assign({}, estimationConfig);
    return config;
}
...
function mergeKeypointTrackerConfig(userConfig) {
    const mergedConfig = mergeBaseTrackerConfig(constants_1.DEFAULT_KEYPOINT_TRACKER_CONFIG, userConfig);
    mergedConfig.keypointTrackerParams = Object.assign({}, constants_1.DEFAULT_KEYPOINT_TRACKER_CONFIG.keypointTrackerParams);
...
function mergeBoundingBoxTrackerConfig(userConfig) {
    const mergedConfig = mergeBaseTrackerConfig(constants_1.DEFAULT_BOUNDING_BOX_TRACKER_CONFIG, userConfig);
    return mergedConfig;
}
