...
function determineNextCropRegion(currentCropRegion, keypoints, keypointIndexByName, imageSize) {
    ...
    for (const key of constants_1.COCO_KEYPOINTS) {
        targetKeypoints[key] = [
    ...
    if (torsoVisible(keypoints, keypointIndexByName)) {
        ...
        const [maxTorsoYrange, maxTorsoXrange, maxBodyYrange, maxBodyXrange] = determineTorsoAndBodyRange(keypoints, keypointIndexByName, targetKeypoints, centerY, centerX);
        let cropLengthHalf = Math.max(maxTorsoXrange * 1.9, maxTorsoYrange * 1.9, maxBodyYrange * 1.2, maxBodyXrange * 1.2);
