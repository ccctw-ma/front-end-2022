...
function computeRotation(landmarks, imageSize) {
    ...
    const rotation = (0, image_utils_1.normalizeRadians)(Math.PI / 2 - Math.atan2(-(y1 - y0), x1 - x0));
    return rotation;
}
/**
...
function handLandmarksToRect(landmarks, imageSize) {
    ...
    const reverseAngle = (0, image_utils_1.normalizeRadians)(-rotation);
    ...
    for (const landmark of landmarks) {
        ...
        const projectedX = originalX * Math.cos(reverseAngle) - originalY * Math.sin(reverseAngle);
        const projectedY = originalX * Math.sin(reverseAngle) + originalY * Math.cos(reverseAngle);
