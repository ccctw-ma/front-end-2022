...
const imagenet_classes_1 = require("./imagenet_classes");
...
function load(modelConfig = {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const mobilenet = new MobileNetImpl(versionStr, alphaStr, modelConfig.modelUrl, inputMin, inputMax);
        yield mobilenet.load();
        return mobilenet;
    });
}
exports.load = load;
class MobileNetImpl {
    ...
    load() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.modelUrl) {
                this.model = yield tfconv.loadGraphModel(this.modelUrl);
                // Expect that models loaded by URL should be normalized to [-1, 1]
            }
            else {
                const url = MODEL_INFO[this.version][this.alpha].url;
                this.model = yield tfconv.loadGraphModel(url, { fromTFHub: true });
            }
            // Warmup the model.
            const result = tf.tidy(() => this.model.predict(tf.zeros([1, IMAGE_SIZE, IMAGE_SIZE, 3])));
            yield result.data();
            result.dispose();
        });
    }
    ...
    infer(img, embedding = false) {
        return tf.tidy(() => {
            ...
            const normalized = tf.add(tf.mul(tf.cast(img, 'float32'), this.normalizationConstant), this.inputMin);
            // Resize the image to
    ...
    classify(img, topk = 3) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const classes = yield getTopKClasses(logits, topk);
            logits.dispose();
            return classes;
        });
    }
...
function getTopKClasses(logits, topK) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const values = yield softmax.data();
        softmax.dispose();
