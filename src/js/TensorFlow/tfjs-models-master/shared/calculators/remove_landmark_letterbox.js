...
function removeLandmarkLetterbox(rawLandmark, padding) {
    ...
    const outLandmarks = rawLandmark.map(landmark => {
        return Object.assign(Object.assign({}, landmark), { x: (landmark.x - left) / (1 - leftAndRight), y: (landmark.y - top) / (1 - topAndBottom), z: landmark.z / (1 - leftAndRight) // Scale Z coordinate as X.
         });
    });
    return outLandmarks;
}
