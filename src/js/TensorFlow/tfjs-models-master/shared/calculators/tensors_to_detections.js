...
function tensorsToDetections(detectionTensors, anchor, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const outputDetections = yield convertToDetections(boxes, normalizedScore, config);
...
function convertToDetections(detectionBoxes, detectionScore, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const detectionBoxesData = yield detectionBoxes.data();
        const detectionScoresData = yield detectionScore.data();
        for (let i = 0; i < config.numBoxes; ++i) {
            ...
            const detection = convertToDetection(detectionBoxesData[boxOffset + 0] /* boxYMin */, detectionBoxesData[boxOffset + 1] /* boxXMin */, detectionBoxesData[boxOffset + 2] /* boxYMax */, detectionBoxesData[boxOffset + 3] /* boxXMax */, detectionScoresData[i], config.flipVertically, i);
    ...
    return tf.tidy(() => {
        ...
        if (config.numKeypoints) {
            for (let k = 0; k < config.numKeypoints; ++k) {
                ...
                const keypointXNormalized = tf.add(tf.mul(tf.div(keypointX, config.xScale), anchor.w), anchor.x);
                const keypointYNormalized = tf.add(tf.mul(tf.div(keypointY, config.yScale), anchor.h), anchor.y);
                boxes = tf.concat([
