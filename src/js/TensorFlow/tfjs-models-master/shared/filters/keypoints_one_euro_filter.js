...
class KeypointsOneEuroFilter {
    ...
    initializeFiltersIfEmpty(keypoints) {
        if (this.xFilters == null || this.xFilters.length !== keypoints.length) {
            this.xFilters = keypoints.map(_ => new one_euro_filter_1.OneEuroFilter(this.config));
            this.yFilters = keypoints.map(_ => new one_euro_filter_1.OneEuroFilter(this.config));
            this.zFilters = keypoints.map(_ => new one_euro_filter_1.OneEuroFilter(this.config));
        }
    }
}
