...
describe('Relative velocity filter ', () => {
    it('Smoke.', () => {
        const filter = new relative_velocity_filter_1.RelativeVelocityFilter({ windowSize: 1, velocityScale: 1 });
        const timestamp1 = 1;
    ...
    it('Same value scale different velocity scales legacy.', () => {
        ...
        const filter1 = new relative_velocity_filter_1.RelativeVelocityFilter({ windowSize: 5, velocityScale: 45 });
        // Less sensitive filter.
        const filter2 = new relative_velocity_filter_1.RelativeVelocityFilter({ windowSize: 5, velocityScale: 0.1 });
        let result1;
        let result2;
        let value;
        const valueScale = 1;
    ...
    it('Different constant value scales same velocity scale legacy.', () => {
        ...
        const filter1 = new relative_velocity_filter_1.RelativeVelocityFilter({ windowSize: 3, velocityScale: sameVelocityScale });
        const filter2 = new relative_velocity_filter_1.RelativeVelocityFilter({ windowSize: 3, velocityScale: sameVelocityScale });
        let result1;
        let result2;
        let value;
    ...
    it('Translation invariance.', () => {
        ...
        const originalPointsFilter = new relative_velocity_filter_1.RelativeVelocityFilter({ windowSize, velocityScale });
        const translatedPointsFilter = new relative_velocity_filter_1.RelativeVelocityFilter({ windowSize, velocityScale });
        ...
        let timestamp = 0;
        for (let iteration = 0; iteration < originalDataPoints.length; ++iteration, timestamp += timeDelta) {
            const originalDataPoint = originalDataPoints[iteration];
            const filteredOriginalValue = originalPointsFilter.apply(originalDataPoint.value, timestamp, originalDataPoint.scale);
            const translatedDataPoint = translatedDataPoints[iteration];
            const actualFilteredTranslatedValue = translatedPointsFilter.apply(translatedDataPoint.value, timestamp, translatedDataPoint.scale);
            const expectedFilteredTranslatedValue = filteredOriginalValue + valueOffset;
            const difference = Math.abs(actualFilteredTranslatedValue - expectedFilteredTranslatedValue);
            if (iteration === 0) {
