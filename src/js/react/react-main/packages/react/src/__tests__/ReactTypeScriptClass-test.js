...
const internalAct = require("jest-react");
// Before Each
let container;
let root;
...
class ForceUpdateWithNoState extends React.Component {
    constructor() {
        super(...arguments);
        this.mutativeValue = this.props.initialValue;
    }
    handleClick() {
...
describe('ReactTypeScriptClass', function () {
    ...
    it('throws if no render function is defined', function () {
        expect(() => {
            expect(() => act(() => root.render(React.createElement(Empty)))).toThrow();
        }).toErrorDev([
    ...
    it('renders based on state using props in the constructor', function () {
        ...
        test(React.createElement(StateBasedOnProps, { initialValue: 'foo', ref: ref }), 'DIV', 'foo');
    ...
    it('warns if getDerivedStateFromProps is not static', function () {
        ...
        expect(function () {
            act(() => root.render(React.createElement(Foo, { foo: 'foo' })));
        }).toErrorDev('Foo: getDerivedStateFromProps() is defined as an instance method ' +
    ...
    it('warns if getDerivedStateFromError is not static', function () {
        ...
        expect(function () {
            act(() => root.render(React.createElement(Foo, { foo: 'foo' })));
        }).toErrorDev('Foo: getDerivedStateFromError() is defined as an instance method ' +
            'and will be ignored. Instead, declare it as a static method.');
    });
    it('warns if getSnapshotBeforeUpdate is static', function () {
        ...
        expect(function () {
            act(() => root.render(React.createElement(Foo, { foo: 'foo' })));
        }).toErrorDev('Foo: getSnapshotBeforeUpdate() is defined as a static method ' +
    ...
    it('warns if state not initialized before static getDerivedStateFromProps', function () {
        ...
        expect(function () {
            act(() => root.render(React.createElement(Foo, { foo: 'foo' })));
        }).toErrorDev('`Foo` uses `getDerivedStateFromProps` but its initial state is ' +
    ...
    it('should warn with non-object in the initial state property', function () {
        expect(() => test(React.createElement(ArrayState), 'SPAN', '')).toErrorDev('ArrayState.state: must be set to an object or null');
        expect(() => test(React.createElement(StringState), 'SPAN', '')).toErrorDev('StringState.state: must be set to an object or null');
        expect(() => test(React.createElement(NumberState), 'SPAN', '')).toErrorDev('NumberState.state: must be set to an object or null');
    });
    ...
    it('warns when classic properties are defined on the instance, ' +
        ...
        expect(() => test(React.createElement(ClassicProperties), 'SPAN', 'foo')).toErrorDev([
    ...
    it('should warn when misspelling shouldComponentUpdate', function () {
        expect(() => test(React.createElement(MisspelledComponent1), 'SPAN', 'foo')).toErrorDev('Warning: ' +
    ...
    it('should warn when misspelling componentWillReceiveProps', function () {
        expect(() => test(React.createElement(MisspelledComponent2), 'SPAN', 'foo')).toErrorDev('Warning: ' +
    ...
    it('should warn when misspelling UNSAFE_componentWillReceiveProps', function () {
        expect(() => test(React.createElement(MisspelledComponent3), 'SPAN', 'foo')).toErrorDev('Warning: ' +
    ...
    it('should throw AND warn when trying to access classic APIs', function () {
        ...
        expect(() => expect(() => ref.current.replaceState({})).toThrow()).toWarnDev('replaceState(...) is deprecated in plain JavaScript React classes', { withoutStack: true });
        expect(() => expect(() => ref.current.isMounted()).toThrow()).toWarnDev('isMounted(...) is deprecated in plain JavaScript React classes', { withoutStack: true });
    });
